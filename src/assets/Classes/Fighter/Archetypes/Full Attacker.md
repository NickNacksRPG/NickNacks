Feeling nostalgic about 3e, 3.5e or PF1e? Well here's me doing my best to recreate that experience in a slightly less heavy handed way than BB fighter. The full attacker is the fightiest guy, all about gaining insane amounts of movement speed and then using that speed to gain more actions. Woe to those who dare to stand in your face.
## <span style="color:rgb(103, 254, 120)">1 - Who's Next?</span>
If you defeat an enemy and don't have enough speed to reach a new enemy, you can move until you threaten the next closest enemy, unless your speed is 0.
## <span style="color:rgb(103, 254, 120)">1 - Full Attack</span>
Any turn you spend your standard action to make an attack (or [[Multiattack]]), you can make an additional <span style="color:rgb(192, 0, 0)">MELEE </span>Superior Attack a turn by sacrificing 45 speed once per turn.

<span style="color:rgb(192, 0, 0)">You cannot use techniques or special attacks after full attacking. If you use a special attack or technique during your turn, you cannot full attack.</span>
## <span style="color:rgb(103, 254, 120)">1 - Fervor of Battle</span>
During combat, your speed increases by 15 this level and another 5 every level after. You must always end your movement with an enemy in reach. If you ever spend a turn without an enemy in [[reach]] for any reason, you become [[clumsy]] 1.
## <span style="color:rgb(103, 254, 120)">2 - Action Surge</span>
Each one can be used in place of a superior attack and they all share a global cooldown, only able to be used 1/round.
You cannot sacrifice superior attacks gained from _baby slayer_ to use these abilities.
- <span style="color:rgb(103, 254, 120)">5 Steps Ahead</span> ([[elusive]]) - [[Prepare]] for an incoming attack. While preparing you can trigger this ability as an immediate action, moving up to 30 feet to the opposite side of a creature that affects you with a harmful effect. You get a +4 bonus to AC and saves against this effect.
- <span style="color:rgb(103, 254, 120)">Random Bullshit Go</span> - Throw an item that isn't a weapon. [[Improvised Weapons]] can still be used. If you throw a harmful item, such as a [[+Toxins|toxin]], it applies [[On-hit]] (then forcing saving throws as normal). If you throw a helpful item, such as a [[+Potions|potions]], its effects function as normal, ignoring any broken vials that may get involved.
  If for whatever reason an effect requires multiple rounds to take effect, that is still the case for this ability.
- <span style="color:rgb(103, 254, 120)">Cunning-ish Action</span> - You can [[disengage]], [[Jump]], [[aid]], [[focus]] or make a [[+Combat Maneuver]].
- <span style="color:rgb(103, 254, 120)">Brute Force</span> - Roll a [[Brute Force]] check.
## <span style="color:rgb(103, 254, 120)">11 - Fullest Attack</span>
When you full attack, you can sacrifice an additional 45 speed to make an additional attack with the same restrictions.